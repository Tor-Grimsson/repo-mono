...
[Overview](#overview) · [Token Architecture](#token-architecture) · [Surfaces](#surfaces--containers) · [Foreground Pairing](#foreground-pairing-on--convention) · [Opacity Scale](#opacity-scale) · [Border Utilities](#border-utilities) · [Elevation](#elevation-system) · [States](#state-variants) · [Utility Classes](#utility-classes-reference) · [Migration](#migration-guide) · [Troubleshooting](#troubleshooting)
...
The system is built on semantic surface tiers, the alpha **`fg`** scale and the opaque **`oq`** scale (mirrors of each other — 14 stops, standard + inverse), accent + status tokens, and the data-viz palette. (The legacy `--kol-opacity-hex-*`, `--kol-container-*`, and `--kol-color-neutral-*` families were removed 2026-05-27 — see the `oq` section below.)
...
/* Contrast Token - For gradient transitions */
--kol-surface-contrast               /* #F2F2F2 (light) / #0B0B0C (dark) */
```

#### Container Tokens — REMOVED (2026-05-27)
The `--kol-container-*` tier was deleted: it duplicated the surface tiers with no added behavior. Use `--kol-surface-secondary` (raised cards) / `--kol-surface-tertiary` (elevated), or the `oq` opaque scale for a specific grey step.
...
Surfaces are the opaque background tiers for both layout and components. (The separate component-level `--kol-container-*` tier was removed — surfaces cover both now.)
...
| Use Case | Token |
|----------|-------|
| Page background | `--kol-surface-primary` |
| Hero / raised section, card background | `--kol-surface-secondary` |
| Modal / dropdown / tooltip (elevated) | `--kol-surface-tertiary` |
| Inverted section (nav/footer) | `--kol-surface-inverse` |
| A specific opaque grey step | `oq` scale (`bg-oq-*`) |

**Rule of thumb:** reach for `--kol-surface-*` for any opaque background; use the `oq` scale when you need a specific grey step over a known surface.
...
.hover\:bg-fg-01:hover   /* 1% on hover */
.hover\:bg-fg-02:hover   /* 2% on hover */
.hover\:bg-fg-04:hover   /* 4% on hover */
.hover\:bg-fg-08:hover   /* 8% on hover */
.hover\:bg-fg-12:hover   /* 12% on hover */
.hover\:bg-fg-16:hover   /* 16% on hover */
.hover\:bg-fg-24:hover   /* 24% on hover */
.hover\:bg-fg-32:hover   /* 32% on hover */
.hover\:bg-fg-48:hover   /* 48% on hover */
.hover\:bg-fg-64:hover   /* 64% on hover */
.hover\:bg-fg-80:hover   /* 80% on hover */
.hover\:bg-fg-88:hover   /* 88% on hover */
.hover\:bg-fg-96:hover   /* 96% on hover */
```
...
### Opaque Scale — `oq` (replaces Opacity-Hex)

> **The `--kol-opacity-hex-*` family was removed (2026-05-27)** and replaced by the **`oq` opaque scale** — the solid mirror of `--kol-fg-*`: same 14 stops, `color-mix()` of the ink onto the **surface** instead of `transparent`, so it's opaque where `fg` is alpha. Auto-themes (no hardcoded light/dark). Classes: `bg-oq-*` / `text-oq-*` / `border-oq-*` (+ `-inverse`, + `hover:`), defined in `packages/theme/kol-opaque.css`. Full reference: [Color Cheat Sheet](./02-colors-cheat-sheet.md).
>
> The legacy hardcoded-hex tables below are retained for migration reference only — **those tokens and classes no longer ship.**
...
**Hover Utilities (v3.5+):**

Hover state classes for opacity-hex backgrounds, available in two formats:

```css
/* Tailwind-style (with colon) */
.hover\:bg-opacity-hex-01:hover    .hover\:bg-opacity-hex-02:hover    .hover\:bg-opacity-hex-04:hover
.hover\:bg-opacity-hex-08:hover    .hover\:bg-opacity-hex-12:hover    .hover\:bg-opacity-hex-16:hover
.hover\:bg-opacity-hex-24:hover    .hover\:bg-opacity-hex-32:hover    .hover\:bg-opacity-hex-64:hover
.hover\:bg-opacity-hex-80:hover    .hover\:bg-opacity-hex-88:hover    .hover\:bg-opacity-hex-96:hover

/* Shorthand (without colon) */
.hover-opacity-hex-01:hover    .hover-opacity-hex-02:hover    .hover-opacity-hex-04:hover
.hover-opacity-hex-08:hover    .hover-opacity-hex-12:hover    .hover-opacity-hex-16:hover
.hover-opacity-hex-24:hover    .hover-opacity-hex-32:hover    .hover-opacity-hex-64:hover
.hover-opacity-hex-80:hover    .hover-opacity-hex-88:hover    .hover-opacity-hex-96:hover

/* Inverse hover variants */
.hover\:bg-opacity-hex-inverse-01:hover    .hover\:bg-opacity-hex-inverse-02:hover    .hover\:bg-opacity-hex-inverse-04:hover
.hover\:bg-opacity-hex-inverse-08:hover    .hover\:bg-opacity-hex-inverse-12:hover    .hover\:bg-opacity-hex-inverse-16:hover
.hover\:bg-opacity-hex-inverse-24:hover    .hover\:bg-opacity-hex-inverse-32:hover    .hover\:bg-opacity-hex-inverse-64:hover
.hover\:bg-opacity-hex-inverse-80:hover    .hover\:bg-opacity-hex-inverse-88:hover    .hover\:bg-opacity-hex-inverse-96:hover
```
...
<!-- Hover state example -->
<button class="bg-surface-inverse hover-opacity-hex-08">
  Button with subtle hover
</button>
```
...
The palette is a shared set of named colors used across tags, charts, and data visualization. Each color has two variants:

- **Saturated** (`--kol-palette-*`) — vivid, used as tag backgrounds in dark mode and chart fills
- **Light** (`--kol-palette-*-light`) — muted/pastel, used as tag backgrounds in light mode

### Token Pairs

| Color | Saturated | Light |
|-------|-----------|-------|
| Blue | {#3740D3} `--kol-palette-blue` | {#c8caf4} `--kol-palette-blue-light` |
| Teal | {#49a0a2} `--kol-palette-teal` | {#d0e8e9} `--kol-palette-teal-light` |
| Green | {#66a44c} `--kol-palette-green` | {#d4e6cb} `--kol-palette-green-light` |
| Yellow | {#ffe32e} `--kol-palette-yellow` | {#d0d79d} `--kol-palette-yellow-light` |
| Red | {#ce4646} `--kol-palette-red` | {#f0caca} `--kol-palette-red-light` |
| Orange | {#db8000} `--kol-palette-orange` | {#f5ddb3} `--kol-palette-orange-light` |
| Purple | {#9437FF} `--kol-palette-purple` | {#e0c8ff} `--kol-palette-purple-light` |

Both variants are defined in `:root` (not theme-scoped), so they are always available regardless of the active theme.

### Tag Color Classes

Tags use `.tag--{color}` classes that automatically select the right background and text color per theme:

| Class | Dark mode (default) | Light mode |
|-------|---------------------|------------|
| `.tag--blue` | Saturated bg + white text | Light bg + dark text |
| `.tag--teal` | Saturated bg + white text | Light bg + dark text |
| `.tag--green` | Saturated bg + white text | Light bg + dark text |
| `.tag--yellow` | Saturated bg + **dark text** | Light bg + dark text |
| `.tag--red` | Saturated bg + white text | Light bg + dark text |
| `.tag--orange` | Saturated bg + white text | Light bg + dark text |
| `.tag--purple` | Saturated bg + white text | Light bg + dark text |
| `.tag--dark` | on-primary bg + surface text | on-primary bg + surface text |

Yellow uses dark text in both modes because its background is too bright for white text.

### Tag Color Assignment

Tags are assigned colors via `getTagColor(tag)` in `apps/web/src/utils/docsHelpers.js`, which hashes the tag string and picks from the palette: `['blue', 'teal', 'green', 'yellow', 'red', 'orange', 'purple', 'dark']`.

### File Locations

- Token definitions: `packages/ui/theme.css` (`:root` block)
- Tag classes: `packages/ui/css/components.css` (`.tag--*`)
- Color assignment: `apps/web/src/utils/docsHelpers.js` (`getTagColor`)
...
.bg-surface-contrast      /* Contrast background for gradients (#F2F2F2 light / #0B0B0C dark) */
```
...
**Hover Border (foreground opacity):**
```css
...
- [Design System Overview](INDEX.md) - Previous version overview and system foundation
- [Design System: Typography](03-typography.md) - Typography implementation following color system patterns
- [Design System: Breakpoints](05-breakpoints.md) - Responsive tiers and layout patterns
- [Foundation: CSS Architecture](../01-foundation/06-css-architecture.md) - Layer model and component architecture

### Implementation Guides
- 3.0.0 CSS Architecture - CSS architecture and best practices
- 4.0.0 Styleguide - Styleguide development and maintenance

### Research Foundation
- [6.1.0 Research: Data Tables](01-data-tables.md) - Color usage in data contexts

### Historical References
- Design System Analysis - Original analysis and research
- Color System Refactor Plan - Phase 1-6 implementation plan

### Supporting Documentation
- 2.1 Color Tables - Quick reference tables (archived, now in this document)
...
**Version History:**
- v3.5 (2025-12-03): Added `--kol-surface-contrast` token (#F2F2F2 light / #0B0B0C dark), `.bg-surface-contrast` utility, hover classes for opacity-hex (`hover:bg-opacity-hex-*`, `hover-opacity-hex-*`, and inverse variants)
