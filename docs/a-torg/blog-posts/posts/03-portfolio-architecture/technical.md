...
Rich content is rendered using Portable Text:

```jsx
// apps/web/src/components/portable-text/PortableTextRenderer.jsx
...
const components = {
  types: {
    image: ({ value }) => (
      <img
        src={urlFor(value).width(800).height(600).fit('crop').url()}
        alt={value.alt || ''}
      />
    )
  },
  marks: {
    link: ({ children, value }) => {
      const rel = value.href.startsWith('http') ? 'noreferrer' : undefined;
      return (
    ...
    <button
      className={`
        ...
        ${variant === 'primary'
          ...
          : 'bg-transparent text-content-primary border border-border-primary hover:bg-surface-secondary'
        }
      `}
      {...props}
    >
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center bg-surface-primary">
      <div className="container mx-auto px-6">
        <h1 className="text-display-xl font-display text-content-primary">
          Kolkrabbi
        </h1>
        <p className="text-h3 font-heading text-content-secondary mt-4">
...
```jsx
// components/animation/AnimatedTitle.jsx
...
```jsx
// components/media/OptimizedImage.jsx
...
**Result:** Lighthouse scores consistently above 90 for performance, accessibility, and SEO.
...
React, Vite, Sanity—none of these are cutting edge, and that's the point. We want to spend innovation tokens on design and UX, not tooling.

### Invest in Documentation

Our `LLM_RULES.md` and `AGENT-CONTEXT.md` files ensure anyone (human or AI) can understand the architecture in minutes.

### Make Design Decisions Once

Design System 2.0 means we never debate colors, spacing, or typography again. The tokens are the decisions.

### Test in Production Early

We deployed incomplete features behind feature flags. Early production exposure catches integration issues before they compound.

### Optimize for Change

Content models will evolve. Design tokens will expand. The architecture accommodates this by keeping concerns separated and dependencies explicit.

### Don't Fear Migration

We consolidated four repos into one while the site was live. Incremental migration with clear phases meant we never lost momentum.
...
Every technical decision is a product decision. We chose this architecture not because it's the most advanced or the trendiest, but because it serves our goals:
...
9. Agent Context - `docs/llm-context/AGENT-CONTEXT.md`
10. Rules & Structure - `docs/llm-context/LLM_RULES.md`