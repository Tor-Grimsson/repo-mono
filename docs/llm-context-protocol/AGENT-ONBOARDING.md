...
2. **Review** `docs/llm-context-protocol/AGENT-CONTEXT.md` section *"Current Focus"* → high-level project status.
3. **Open** the latest file in `docs/llm-context-protocol/session-logs/` (current cycle only) for tactical details.
4. **Check** `docs/status/architectural-decisions-log.md` for architectural constraints or open questions.
5. **Confirm** any task-specific briefs in `docs/components/components-hierarchy-migration-plan.md` or other living planning docs (search `docs/` for "Status: In Progress").
...
- Log every **major milestone** or **context hand-off** using `docs/llm-context-protocol/session-logs/TEMPLATE.md`.
- Keep only the **latest working day’s checkpoints** in `docs/llm-context-protocol/session-logs/`.
...
- **Canonical status:** `docs/llm-context-protocol/AGENT-CONTEXT.md` – update after shipping a feature or closing a workstream.
...
| Trigger | Action |
|--------|--------|
| Finished a feature | Update `docs/llm-context-protocol/AGENT-CONTEXT.md` → “Active Focus” + adjust milestone notes |
...
| Daily work session end | Create checkpoint log in `docs/llm-context-protocol/session-logs/` + roll older logs into archive |
...
2. Update `docs/llm-context-protocol/AGENT-CONTEXT.md` (status + current focus).
