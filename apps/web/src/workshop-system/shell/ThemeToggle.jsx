...
function getInitialTheme() {
  ...
  // Trust what the boot script already applied (class or data-attr); default light.
 ...
 *
 * Self-contained: owns theme state, writes `data-theme` + the `.dark` class to
 * <html>, persists to localStorage under `theme` (shared with the index.html
 * boot script, so the choice survives reload). The icon-swap animation slides a pair
...
export default function ThemeToggle({ variant = 'icon', className = '' }) {
  ...
  useEffect(() => {
    ...
    // Also drive the `.dark` class — the KOL remap is :is([data-theme=dark], .dark),
    // so the class must flip too or the leftover `.dark` keeps dark mode stuck on.
