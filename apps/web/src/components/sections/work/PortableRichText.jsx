...
const components = {
  marks: {
    link: ({ children, value }) => {
      ...
      return (
        <a
          href={href}
          className="underline decoration-[var(--kol-border-default)] decoration-2 underline-offset-4 transition-opacity hover:opacity-70"
  ...
  types: {
    callout: ({ value }) => (
      <div className="rounded-3xl border border-[var(--kol-border-default)] bg-[var(--kol-surface-secondary)] p-6">
  ...
  block: {
    ...
    blockquote: ({ children }) => (
      <blockquote className="border-l-4 border-[var(--kol-border-default)] pl-6 italic opacity-70">
        {children}
      </blockquote>
    )
  },
...
export default function PortableRichText({ value = [] }) {
  if (!value || value.length === 0) {
    return (
      <div className="rounded-3xl border border-dashed border-[var(--kol-border-default)] bg-[var(--kol-surface-secondary)] p-6 text-sm opacity-70">
