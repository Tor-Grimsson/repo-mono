...
/**
 * FlikB Portrait V4 — Radial breathing pulse from center
 * Cream chevrons on raspberry. 2160x3600, 450 frames, 30fps.
 */
export const FlikBV4_P: React.FC = () => {
      ...
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}>
        {Array.from({ length: ROWS + 2 }).map((_, row) =>
          Array.from({ length: COLS + 2 }).map((_, col) => {
            ...
            const appearDelay = dist * 2;
            const appearP = ease(progress(frame, appearDelay, appearDelay + 16));
