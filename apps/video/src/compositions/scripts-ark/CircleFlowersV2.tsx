...
// Square 1080x1080
// 0-120:   Quick build
// 120-390: 9s color cycling ripple
// 390-600: Hold
// ================================================================

export const CircleFlowersV2: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const SCALE = 4.0;

  // Quick build: all 7 in 120 frames (~17f each)
  ...
  const getCircleFill = (index: number): string => {
    if (frame < 120) return WARM_FILLS[index];
    if (frame >= 390) return WARM_FILLS[index];

    const cycleFrame = frame - 120;
    const stagger = index * 6;
    const cycleDuration = 67;
