...
/**
 * FlikB Portrait V1 — Row-by-row staggered reveal + column flip
 * Cream chevrons on raspberry. 2160x3600, 450 frames, 30fps.
 */
export const FlikB_P: React.FC = () => {
      ...
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}>
        {Array.from({ length: ROWS + 2 }).map((_, row) =>
          Array.from({ length: COLS + 2 }).map((_, col) => {
            ...
            const drawDelay = row * 2 + col * 1.5;
            const drawP = ease(progress(frame, drawDelay, drawDelay + 18));

            const flipStart = 120;
            const shouldFlip = col % 2 === 1;
            const flipProgress = shouldFlip
              ? spring({
                  frame: Math.max(0, frame - flipStart - col * 3),
                  fps,
