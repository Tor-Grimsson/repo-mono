...
/**
 * FlikB V3 — Column cascade + vertical interlock
 * Cream chevrons on raspberry. 1920x1080, 450 frames, 30fps.
 */
export const FlikBV3: React.FC = () => {
        ...
        {Array.from({ length: ROWS + 2 }).map((_, row) =>
          Array.from({ length: COLS + 2 }).map((_, col) => {
            ...
            const slideYProgress = frame > slideStart
              ? spring({
                  frame: frame - slideStart - col * 1.5,
                  fps,
