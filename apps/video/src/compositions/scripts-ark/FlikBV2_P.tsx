...
/**
 * FlikB Portrait V2 — Wave ripple
 * Cream chevrons on raspberry. 2160x3600, 450 frames, 30fps.
 */
export const FlikBV2_P: React.FC = () => {
      ...
      <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`}>
        {Array.from({ length: ROWS + 2 }).map((_, row) =>
          Array.from({ length: COLS + 2 }).map((_, col) => {
            ...
            // Vertical wave (travels top to bottom for portrait)
            const wavePos = (frame - 40) * waveSpeed - row * 0.3 - col * 0.4;
