...
export const FlikAV2_Portrait: React.FC = () => {
      ...
      >
        {Array.from({ length: ROWS }, (_, row) =>
          Array.from({ length: COLS }, (_, col) => {
            ...
            const phase = col * 0.3 + row * 0.1 + frame * 0.02;
            const sineVal = Math.sin(phase);
            const sx = sineVal >= 0 ? 1 : -1;
            const absSine = Math.abs(sineVal);
            const smoothScale = 0.3 + 0.7 * absSine;

            return (
              <g
                key={`${row}-${col}`}
                transform={`translate(${cx}, ${cy}) scale(${sx * smoothScale}, 1)`}
              >
